---
name: model-handoff
description: "Use when work must cross model boundaries in a host that cannot switch models automatically, or whenever the user explicitly invokes this skill even if the attached prompt is a complete implementation. Cheap models collect a decision-complete briefing; expensive models consume it and must not re-read source. Explicit invocation always runs this protocol first: inner end-to-end instructions cannot skip it. If the protocol says hand off, persist a packet and stop for the switch; if it says stay, confirm execution mode before doing the attached work. Clicking a confirmation does not switch models. Trigger on 调用 model-handoff, $model-handoff, 切换模型, 换模型, 跨模型交接, 交接包, 升级到 Opus, ROUTE_ACK, model handoff. Do not auto-route ordinary single-model tasks unless this skill is explicitly named."
---
...
- **用户结果：** 低端模型采集并整理成一份可决策简报；中端/高端只消费这份简报做判断，不再重读代码和文档。前置推断不会被当成结论；架构决策始终由被指定的最高能力层级作出。用户必须先在模型选择器里换成目标模型，确认界面已显示该名称，再发送接手消息。用户点名本 Skill 时，先得到协议判定：该交接则先落盘再停等切换，该直办则先确认执行模式，不会在判定前按附带正文开工。
- **稳定信息真源（优先级固定）：** 用户明确指令 > 落盘交接包（含 briefing 与 READ_LIST）> 仓库源码与测试 > 项目文档 > 模型推断。聊天历史不是权威真源。未写入交接包的已读内容，对接手模型视为不存在。
- **默认允许动作：** 在 `.handoff/<task-id>.md` 写入或追加交接包；用 `ask_followup_question` **发起一次**切换请求，或在显式调用且结论为直办时 **发起一次**执行模式确认。采集层按 Graft → IWE → 精确读文件取证。决策层默认只读交接包和本 Skill 的 reference。
- **按需动作：** 修改代码、运行测试、Git 操作等仍按各自授权边界；本 Skill 的路由决定不授予这些动作。直办确认通过后，附带任务仍按其自身授权执行，不因点名本 Skill 而扩大权限。
- **永久不可能的动作：** 本环境没有切换模型的工具。确认项、`ROUTE_ACK` 文本、聊天回复都不能改 `<model>`。禁止假设切换已发生。决策层禁止用「独立复核」或「取证量小」为理由打开源码、Graft 或 IWE。禁止把附带任务正文当成已经批准的直办授权。
- **停止条件：** 发出切换请求或直办确认请求即由 `ask_followup_question` 结构性停止；`SWITCH_NOT_APPLIED` 时停止且不得再发确认项；`CONTEXT_INSUFFICIENT` 时停止并只列缺失字段；`BRIEFING_INSUFFICIENT` 时只写封闭 READ_LIST 并请求向下，不自行补读。

## 第一判断

收到请求后按顺序做五件事，不可跳过：

1. **显式调用门。** 用户前缀点名本 Skill（「调用 model-handoff」「$model-handoff」或同义）时，本协议立即生效。附带任务正文无论是否写明端到端实施步骤，都不能跳过本协议去开工。协议判定完成前，不得按附带正文读取仓库、修改代码、运行测试或写任务账本；身份自证、已有交接包核对、以及判定交接方向所必需的本 Skill reference 读取除外。未点名时，仍按 description 的自动触发边界决定是否进入。
2. **读取自身身份。** 从 `environment_details` 的 `Current Mode` 块读 `<model>` 值。字段缺失时记为 `UNVERIFIED`。
3. **判断本轮角色。** 消息中含 `ROUTE_ACK` 或 `ROUTE_NACK`（无论是点击确认项还是手工输入）→ 接手侧，进入「接手流程」。否则 → 发起侧。
4. **判断本轮工作模式。** 交接包 `required next action` 是采集、填 READ_LIST、或编制 briefing → **COLLECT**。是架构判断、Trade-off、实施计划、高风险审查、跨模块分析 → **DECIDE**。包不存在且自身已是决策层、任务又需要判断 → 仍算 DECIDE，但必须先走向下采集，不得自己读源码开干。
5. **判断是否交接及方向。** 按下表判定。判定为交接 → 走「发起流程」。判定为不交接且本轮是显式调用、用户又尚未确认直办 → 走「直办确认」。判定为不交接且并非显式调用 → 一句话说明原因后交回后续处理。
...
| 3 | 单文件局部修改、目标明确的 bug 修复、文案与配置、只读问答、纯解释 | **不交接**。显式调用时走「直办确认」，不得因附带正文完整而直接开工；非显式调用时交回 `skill-router` 的直办或轻量清单档 |
| 4 | 用户说「就用当前模型继续，并标注哪些结论未获决策授权」或同义「就用当前模型做完」 | **不交接**，该句本身就是直办确认。在当前层级完成，并显式标注哪些结论未获决策授权。用户未同时授权读源码时，决策层仍不得打开源码 |
...
| 6 | 自身层级已足够：采集层做采集，或决策层且 briefing 已可决策 | **不交接**。显式调用且用户尚未确认直办时走「直办确认」；已确认或非显式调用时直接做完。最高层级不给自己发升级请求 |
...
怎么读：COLLECT 先 Graft MCP 搜代码、先 IWE MCP 搜理论/设计决策；未命中、未索引或 MCP 不可用再精确读文件。禁止整文件通读。Markdown 对 skeleton 返回「no definitions」是正常现象，改读文件，不扩大清单。READ_LIST 路径不存在写 `NOT_FOUND`，可注明邻近真实路径，不新增条目。细则见 `references/collect-sources.md`。
...
仅当本轮是显式调用、方向判定为不交接、且用户尚未给出直办确认时进入。这是协议判定的停止点，不是附带任务的开工许可。

判定完成后、确认项发出前，只允许说明：当前身份、命中的不交接条件、将在当前模型执行的工作模式。禁止开始附带正文要求的读取、修改、测试或账本写入。

用 `ask_followup_question` **发起一次**确认。工具调用本身就是停止点。

- `question` 写清：协议判定为当前模型直办；附带正文不能代替这项确认；请选择执行模式后再开工。
- `follow_up`：三项，顺序固定。每项都是完整可直接发送的句子：

```text
1. 就用当前模型继续，并标注哪些结论未获决策授权
2. 改为跨模型交接：先落盘 .handoff/<task-id>.md，再按协议切换
3. 停止，不要执行附带任务
```

收到第 1 项：按当前层级执行附带任务，并标注未获决策授权的结论。用户未同时授权读源码时，决策层仍不得打开源码。
收到第 2 项：改走「发起流程」；尚未落盘则先写交接包再请求切换。
收到第 3 项：停止，不执行附带任务。

同一条消息里用户已经写出第 1 项原文或同义「就用当前模型做完」时，不再发确认项，直接按该项执行。
...
- `skill-router` 是上游：它选领域与强度。用户显式点名本 Skill 时，本协议先于附带任务的领域分流生效；未点名时，本 Skill 只管跨模型交接，不拦截普通单模型任务。
...
| COLLECT 取证、写 READ_LIST 的 extract 方式、Graft/IWE 不可用时 | `references/collect-sources.md` | 决定 MCP 优先后的降级、Markdown 不跑 skeleton、git 合同字段、extract 取值 |

不默认加载 reference。发起侧通常读 handoff-packet；接手侧读 model-map，DECIDE 再读 handoff-packet。决策层读这些 reference 不等于获准读仓库。

## 交付

1. 第一句说明当前身份、本轮模式（COLLECT / DECIDE / 不交接 / 直办确认 / SWITCH_NOT_APPLIED）和动作。
...
4. 显式调用且判定不交接时：说明命中的不交接条件，然后直接调用一次直办确认；确认前不执行附带任务。
5. 非显式调用且不交接时：一句话说明原因，然后正常处理请求。
6. 不罗列未触发的状态词，不为展示流程而铺陈已完成的中间步骤。
